/**
 * Hybrid Search: FTS5 keyword search + optional vector rerank
 * Returns file paths ranked by relevance
 * 
 * Default: FTS5 only (no network calls)
 * Optional: Vector rerank if OPENCLAW_DOCS_EMBEDDINGS=true
 */

const fs = require('fs');
const http = require('http');

// Embeddings are OPT-IN only
...
/**
 * Get query embedding (only if opted-in)
 * 
 * ⚠️ SECURITY NOTE: This sends your query to EMBED_URL.
...
/**
 * Search with keyword filter + optional vector rerank
 */
async function search(indexPath, query, options = {}) {
  ...
  // Step 2: Vector rerank candidates (only if opted-in)
...
function formatResults(results, query) {
  ...
  lines.push(`Mode: ${EMBEDDINGS_ENABLED ? 'Hybrid (FTS5 + Vector)' : 'FTS5 keyword search'}\n`);
